...
class GlumpyGlfwRenderer(FixedProgrammablePipelineRenderer):
    def __init__(self, backend, attach_callbacks=True):
        super(GlumpyGlfwRenderer, self).__init__()
        self.backend = backend
        self.window = backend._id
    ...
    def keyboard_callback(self, window, key, scancode, action, mods):
        ...
        if not io.want_capture_keyboard and not io.want_text_input:
            self.backend._on_key_press(window, key, scancode, action, mods)
    ...
    def char_callback(self, window, char):
        ...
        if not io.want_capture_keyboard and not io.want_text_input:
            self.backend._on_key_char(window, char)
