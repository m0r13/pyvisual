...
vec4 sampleMirroredFrag(vec2 origUV, vec2 uv) {
    if (dInputWrapping == 0) {
        if (uv.x < 0.0 || uv.y < 0.0 || uv.x > 1.0 || uv.y > 1.0) {
            return texture2D(uBackgroundTexture, origUV);
...
vec4 samplePolarMirrorGuide(vec2 polar, vec4 frag) {
    ...
    frag = sampleMirroredFrag(vec2(0.0), polarToUV(adjustedPolar));
...
vec4 filterFrag(vec2 uv, vec4 _) {
    ...
    vec4 frag = sampleMirroredFrag(uv, uv0);
    if (uGuide) {
        ...
        vec4 frag1 = sampleMirroredFrag(uv, uv1);
        if (dBlendMode == 0) {
