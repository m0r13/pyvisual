...
uniform sampler2D uAdditionalTexture;
uniform float uTime;
uniform bool uReset;
...
vec4 tx(in vec2 p){ return texture2D(uInputTexture, p); }
...
vec4 filterFrag(vec2 uv, vec4 frag) {
    ...
    vec2 size = textureSize(uInputTexture, 0);
    vec2 pw = 1./size.xy; // Relative pixel width. Used for neighboring pixels, etc.
    ...
    vec3 noise = hash33(uv + vec2(53, 43)*uTime)*.6 + .2;
    ...
    //float ifr = uReset ? 0.5 : 0.97;
    float ifr = 0.97;
    frag.xy = mix(texture2D(uAdditionalTexture, uv).xz, clamp(vec2(newReactDiff, avgReactDiff/.98), 0., 1.), ifr);
    //frag.xy = clamp(vec2(newReactDiff, avgReactDiff/.98), 0., 1.);
    if (uReset) {
        frag = vec4(noise, 1.0);
    }
    return frag;
